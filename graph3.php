...
{
    return R::getAll('SELECT DISTINCT a.project_id, b.pr_name FROM logs a, projects b WHERE a.project_id = b.id AND a.user_id= ? ORDER BY a.project_id', [$_SESSION["graph_id"]]);
}

function getLogs()
{
    return R::getAll('SELECT a.id, a.project_id, b.pr_name, a.task_id, c.t_name, a.new_status, a.time, a.date FROM logs a, projects b, tasks c WHERE a.project_id = b.id AND a.task_id = c.id  AND a.user_id = ?', [$_SESSION["graph_id"]]);
}
...
try {
    ...
    if (empty($logs)) echo "No activity by " . $_SESSION["graph_email"] . ".";
        ...
        foreach ($activity_date as $day) {
            $temp = 0;
            $sql = "select * from logs where date = '" . $day . "' and user_id = " . $_SESSION["graph_id"] . "";
            $rows = R::getAll($sql);
                    ...
                    foreach ($logs as $log) {
                        if ($log['date'] === $logDate) {
                            echo '<tr>
                                ...
                                <td class="log_user_id" style="text-align: center;">' . $_SESSION["graph_id"] . '</td>
                                <td class="username" style="text-align: center;">' . $_SESSION["graph_email"] . '</td>
